...
Two narrower items survive, and they are *not* the same question:

**(a) The TCR campaign registration is a separate document from wyxr.org.** Carrier filtering keys off the use case, sample messages, and message-flow description submitted to The Campaign Registry through Twilio — not off the station's website terms. Well-drafted public terms are the legal/TCPA basis for sending; they don't retroactively broaden a campaign registered as purely conversational. If the registered samples are all 1:1 DJ replies, one-to-many traffic can still get filtered or throttled — which would degrade *normal DJ replies too*, since it's the same number. **Action:** before the first full send, check the campaign's registered use case in the Twilio Console (Messaging → Regulatory Compliance → A2P Messaging). If it's Conversational-only, the fix is adding a Mixed/Low-Volume-Mixed use case or a second campaign — not a code change. This is a go-live checklist item for staff, downgraded from "hard blocker" to "verify and likely fine," since the underlying consent is documented.

**(b) Fundraising is the one genuinely uncovered case.** The privacy policy's "**solely** … programming, show updates, and DJ interactions" is narrower than the T&C's "occasional promotional messages." A pledge-drive or donation-ask text — a predictable want for a 501(c)(3) — sits in the gap between those two sentences. Programming/event/community announcements are clearly fine; donation solicitation is not clearly authorized by the narrower privacy-policy sentence. If the station wants to text donation asks, align that sentence first. Flagged as Open Question 3, and deliberately *not* something the code should try to police.
...
|---|---|---|
| `'sms'` | `server/routes/webhook.js:165` | Listener texted in and replied YES. Matches T&C §5.2 exactly. Strongest. |
...
2. When the listener replies YES, `server/routes/webhook.js:161-170` sets `opted_in = true` **and rewrites `opt_in_method` to `'sms'`**, sends the confirmation, and writes a `confirm` row to `opt_in_log`.
...
7. **Consent-filter test:** seed local contacts with a mix of `opt_in_method` values (`'sms'`, `'web'`, `'import'` — the last with `opted_in = true`, i.e. the staff-attested case) and confirm that `GET /admin/broadcasts/recipient-count` reports the correct `byMethod` breakdown, that `import` rows appear only in `excludedImported`, and that **no** request parameter or payload can pull them into an actual send. Then exercise the conversion path end-to-end: import a row without the attestation, simulate a `YES` reply to the webhook, assert `opt_in_method` flips to `'sms'` (`webhook.js:161-170`), and confirm the contact now appears in the eligible count (§8.3).
8. **Full send:** only after 1–7 pass, and only after the TCR campaign use-case check in §8.2(a) is done.
...
- A second, explicitly-registered A2P campaign/use-case for announcements, if §8.2 determines the current campaign doesn't cover it
...
2. **TCR campaign use-case verification (checklist item, not a blocker).** Separately from the website terms, confirm in the Twilio Console that the *registered* 10DLC campaign use case covers one-to-many traffic — carriers filter on the TCR registration, not on wyxr.org. If it's Conversational-only, add a Mixed use case or a second campaign. Staff action, not a code change. See §8.2(a).
